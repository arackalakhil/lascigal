...
function lovestory_ready_actions() {
    ...
    jQuery(document).ready(function() {
    jQuery('.open_child_menu').on('click', function(e) {
        e.preventDefault();
        var $parentLi = jQuery(this).closest('li.menu-item-has-children');

        if ($parentLi.hasClass('opened')) {
            $parentLi.removeClass('opened');
            $parentLi.find('ul.sub-menu').slideUp();
        } else {
            jQuery('li.menu-item-has-children.opened').removeClass('opened').find('ul.sub-menu').slideUp();
            $parentLi.addClass('opened');
            $parentLi.find('ul.sub-menu').slideDown();
        }
    });
});
    ...
    jQuery('.woocommerce,.woocommerce-page').on('click', '.mode_buttons a', function(e) {
        ...
        jQuery.cookie('lovestory_shop_mode', mode, {
            expires: 365,
            path: '/'
        });
        ...
        if (!exists) {
            var message_popup = lovestory_message_dialog('<label for="bookmark_title">' + LOVESTORY_STORAGE['strings']['bookmark_title'] + '</label><br><input type="text" id="bookmark_title" name="bookmark_title" value="' + title + '">', LOVESTORY_STORAGE['strings']['bookmark_add'], null,
                function(btn, popup) {
                    ...
                    jQuery.cookie('lovestory_bookmarks', JSON.stringify(list), {
                        expires: 365,
                        path: '/'
                    });
                    setTimeout(function() {
                        lovestory_message_success(LOVESTORY_STORAGE['strings']['bookmark_added'], LOVESTORY_STORAGE['strings']['bookmark_add']);
                    }, LOVESTORY_STORAGE['message_timeout'] / 4);
                });
        } else
...
function lovestory_init_appended_isotope(posts_container, filters) {
    ...
    if (!lovestory_check_images_complete(posts_container) && LOVESTORY_STORAGE['isotope_init_counter']++ < 30) {
        setTimeout(function() {
            lovestory_init_appended_isotope(posts_container, filters);
        }, 200);
        return;
    }
    // Add filters
...
function lovestory_init_media_elements(cont) {
    if (LOVESTORY_STORAGE['media_elements_enabled'] && cont.find('audio,video').length > 0) {
        if (window.mejs) {
            ...
            setTimeout(function() {
                lovestory_init_media_elements(cont);
            }, 400);
    }
}
