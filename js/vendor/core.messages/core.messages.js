...
function lovestory_message(opt) {
    ...
    jQuery('body .lovestory_message').animate({
        top: top + Math.round((jQuery(window).height() - jQuery('.lovestory_message').height()) / 2),
        opacity: 1
    }, {
        complete: function() {
            // Call init function
...
function lovestory_message_destroy() {
    ...
    jQuery('.lovestory_message').animate({
        top: top - jQuery('.lovestory_message').height(),
        opacity: 0
    });
    setTimeout(function() {
        jQuery('#lovestory_modal_bg').remove();
